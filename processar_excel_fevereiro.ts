...
async function processarExcelFevereiro() {
  console.log('📊 Processando arquivo Excel de fevereiro de 2025...');
  
  try {
    // Ler o arquivo Excel
    const workbook = XLSX.readFile('./attached_assets/ANÁLISES PARA CERTIFICAÇÃO e DECLARAÇÕES_____ (5)_1752024102607.xlsx');
    
    // Listar as sheets disponíveis
    console.log('📋 Sheets disponíveis:', workbook.SheetNames);
    
    // Usar a sheet "Fevereiro"
    const sheetName = workbook.SheetNames.find(name => name.toLowerCase() === 'fevereiro') || workbook.SheetNames[0];
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      if (Array.isArray(row)) {
        ...
        if (rowStr.includes('status') || rowStr.includes('aluno') || rowStr.includes('cpf')) {
          headerRowIndex = i;
    ...
    // Mapear colunas
    const colunas = {
      status: encontrarColuna(header, ['status']),
      aluno: encontrarColuna(header, ['aluno', 'nome']),
      cpf: encontrarColuna(header, ['cpf']),
      dataSolicitacao: encontrarColuna(header, ['data solicitação', 'data_solicitacao']),
      ...
      plataforma: encontrarColuna(header, ['plataforma', 'atividades']),
      praticasPedagogicas: encontrarColuna(header, ['práticas pedagógicas', 'praticas pedagogicas']),
      dataLiberacao: encontrarColuna(header, ['data liberação', 'data_liberacao']),
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      const status = inferirStatus(row[colunas.status]?.toString()?.trim() || '');
      ...
      const certificacao = {
        ...
        aluno: aluno,
        cpf: cpf,
        dataSolicitacao: row[colunas.dataSolicitacao]?.toString()?.trim() || '',
        ...
        praticasPedagogicas: row[colunas.praticasPedagogicas]?.toString()?.trim() || '',
        dataLiberacao: row[colunas.dataLiberacao]?.toString()?.trim() || '',
    ...
    const dados = {
      arquivo: 'ANÁLISES PARA CERTIFICAÇÃO e DECLARAÇÕES_____ (5)_1752024102607.xlsx',
      sheet: sheetName,
    ...
    fs.writeFileSync('./dados_fevereiro_2025.json', JSON.stringify(dados, null, 2));
    console.log('\n💾 Dados salvos em: dados_fevereiro_2025.json');
    ...
    console.log('\n📊 Estatísticas por status:');
    Object.entries(estatisticasStatus).forEach(([status, count]) => {
...
function inferirModalidade(curso: string): string {
  ...
  if (curso.includes('360') || curso.includes('400') || curso.includes('620') || curso.includes('720')) {
    return 'Pós-graduação';
  }
  if (curso.includes('1320') || curso.includes('1600')) {
    return 'Segunda licenciatura';
  }
  if (curso.includes('1000') || curso.includes('1200')) {
    return 'Formação pedagógica';
  }
...
function inferirStatus(statusText: string): string {
  ...
  if (statusLower.includes('concluída') || statusLower.includes('certificado') || statusLower.includes('aluno certificado')) {
    return 'concluido';
  }
  if (statusLower.includes('em andamento') || statusLower.includes('processo') || statusLower.includes('certificação')) {
    return 'em_andamento';
  }
  if (statusLower.includes('pendente') || statusLower.includes('aguardando') || statusLower.includes('em análise')) {
    return 'pendente';
  }
  if (statusLower.includes('cancelado') || statusLower.includes('expirado') || statusLower.includes('reprovado')) {
    return 'cancelado';
  }
...
export { processarExcelFevereiro };