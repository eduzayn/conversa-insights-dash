...
async function importarCertificacoesJaneiro2025() {
  console.log('📥 Importando certificações de janeiro 2025...');
  
  try {
    // Carregar dados processados
    const dados = JSON.parse(fs.readFileSync('./dados_janeiro_2025.json', 'utf-8'));
    ...
    fs.writeFileSync('./relatorio_importacao_janeiro_2025.json', JSON.stringify(relatorio, null, 2));
    console.log('\n📄 Relatório salvo em: relatorio_importacao_janeiro_2025.json');
...
function extrairDisciplinasRestantes(plataforma: string): number | null {
  ...
  // Se menciona "todas" ou "finalizou todas"
  if (texto.includes('todas') || texto.includes('finalizou todas') || texto.includes('aprovado em todas')) {
    return 0;
  }
...
function extrairSituacaoAnalise(plataforma: string, documentacao: string): string {
  ...
  if (textoPlataforma.includes('finalizou todas') || textoPlataforma.includes('aprovado em todas')) {
    if (textoDocumentacao.includes('deferida') || textoDocumentacao.includes('aprovada')) {
      return 'aprovado';
    }
  }
...
function getSubcategoria(modalidade: string, curso: string): string {
  ...
  if (modalidadeLower.includes('segunda licenciatura')) {
    ...
    if (cursoLower.includes('educação especial')) {
      return 'educacao_especial';
    }
    return 'outras';
  }
  
  if (modalidadeLower.includes('formação pedagógica')) {
    if (cursoLower.includes('sociologia')) {
      return 'sociologia';
    }
    if (cursoLower.includes('filosofia')) {
      return 'filosofia';
    }
    if (cursoLower.includes('ciências da religião')) {
      return 'ciencias_religiao';
    }
    return 'outras';
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  importarCertificacoesJaneiro2025()
    .then(() => {
      console.log('\n🎉 Importação de janeiro 2025 concluída com sucesso!');
      process.exit(0);
    })
    .catch((error) => {
      console.error('❌ Erro na importação:', error);
...
export { importarCertificacoesJaneiro2025 };