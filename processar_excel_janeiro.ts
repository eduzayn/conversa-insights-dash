...
async function processarExcelJaneiro() {
  console.log('📊 Processando arquivo Excel de janeiro de 2025...');
  
  try {
    // Ler o arquivo Excel
    const workbook = XLSX.readFile('./attached_assets/ANÁLISES PARA CERTIFICAÇÃO e DECLARAÇÕES_____ (7)_1752024345053.xlsx');
    
    // Listar as sheets disponíveis
    console.log('📋 Sheets disponíveis:', workbook.SheetNames);
    
    // Usar a sheet "Janeiro"
    const sheetName = workbook.SheetNames.find(name => name.toLowerCase() === 'janeiro') || workbook.SheetNames[0];
    ...
    const dados = {
      arquivo: 'ANÁLISES PARA CERTIFICAÇÃO e DECLARAÇÕES_____ (7)_1752024345053.xlsx',
      sheet: sheetName,
    ...
    fs.writeFileSync('./dados_janeiro_2025.json', JSON.stringify(dados, null, 2));
    console.log('\n💾 Dados salvos em: dados_janeiro_2025.json');
