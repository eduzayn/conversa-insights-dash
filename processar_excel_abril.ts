...
async function processarExcelAbril() {
  console.log('📊 Processando arquivo Excel de abril de 2025...');
  
  try {
    // Ler o arquivo Excel
    const workbook = XLSX.readFile('./attached_assets/ANÁLISES PARA CERTIFICAÇÃO e DECLARAÇÕES_____ (3)_1752023388012.xlsx');
    
    // Listar as sheets disponíveis
    console.log('📋 Sheets disponíveis:', workbook.SheetNames);
    
    // Usar a sheet "Abril"
    const sheetName = workbook.SheetNames.find(name => name.toLowerCase() === 'abril') || workbook.SheetNames[0];
    ...
    fs.writeFileSync('./dados_abril_2025.json', JSON.stringify(dados, null, 2));
    console.log('\n💾 Dados salvos em: dados_abril_2025.json');
