...
async function processarExcelMarco() {
  console.log('📊 Processando arquivo Excel de março de 2025...');
  
  try {
    // Ler o arquivo Excel
    const workbook = XLSX.readFile('./attached_assets/ANÁLISES PARA CERTIFICAÇÃO e DECLARAÇÕES_____ (4)_1752023930428.xlsx');
    
    // Listar as sheets disponíveis
    console.log('📋 Sheets disponíveis:', workbook.SheetNames);
    
    // Usar a sheet "Março"
    const sheetName = workbook.SheetNames.find(name => name.toLowerCase() === 'março' || name.toLowerCase() === 'marco') || workbook.SheetNames[0];
    ...
    fs.writeFileSync('./dados_marco_2025.json', JSON.stringify(dados, null, 2));
    console.log('\n💾 Dados salvos em: dados_marco_2025.json');
