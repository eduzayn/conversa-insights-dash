...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/admin/asaas/payments/:paymentId/details", authenticateToken, async (req: any, res) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Acesso negado. Apenas administradores." });
      }
      ...
      const paymentDetails = await asaasService.getPaymentDetails(paymentId);
  ...
  // Reenviar cobrança
  app.post("/api/admin/asaas/payments/:paymentId/resend", authenticateToken, async (req: any, res) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Acesso negado. Apenas administradores." });
      }
      ...
      const { paymentId } = req.params;
      const result = await asaasService.resendPayment(paymentId);
      res.json({
        success: true,
  ...
  // Cancelar pagamento
  app.post("/api/admin/asaas/payments/:paymentId/cancel", authenticateToken, async (req: any, res) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Acesso negado. Apenas administradores." });
      }
  ...
  // Estornar pagamento
  app.post("/api/admin/asaas/payments/:paymentId/refund", authenticateToken, async (req: any, res) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Acesso negado. Apenas administradores." });
      }
      ...
      const { paymentId } = req.params;
      const result = await asaasService.refundPayment(paymentId);
      res.json({
        success: true,
